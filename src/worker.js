...
async function calcularFrete(request, env) {
  try {
    ...
    const itens = produtos.map((p, index) => ({
      id: String(p.id || `aurea-${index + 1}`),
      width: saneDim(p.largura || p.width, 12),
      height: saneDim(p.altura || p.height, 12),
      length: saneDim(p.comprimento || p.length, 20),
      weight: saneDim(p.peso || p.weight, 0.6),
      insurance_value: Math.max(0, Number(p.valor ?? p.price ?? 0)),
    ...
    const upstream = await fetch("https://melhorenvio.com.br/api/v2/me/shipment/calculate", {
      ...
      headers: {
        ...
        "User-Agent": "AUREA Perfumes/1.0"
      },
      body: JSON.stringify({ from: { postal_code: String(env.AUREA_ORIGIN_CEP).replace(/\D/g, "") }, to: { postal_code: cep }, products: itens })
    });
    ...
    const fretes = Array.isArray(data) ? data.filter(x => x && (x.price != null || x.custom_price != null)).map(x => ({
      id: x.id ?? null,
      company: x.company?.name || x.company || "Transportadora",
      name: x.name || "Frete",
    ...
    console.error("Frete:", e);
    return resposta({ ok: false, error: "Erro interno ao calcular o frete." }, 500);
  }
}
...
async function criarPagamentoPix(request, env) {
  try {
    ...
    const referencia = `AUREA-${Date.now()}-${crypto.randomUUID().slice(0, 8)}`;
    const payload = {
      transaction_amount: Number(total.toFixed(2)),
      description: `Pedido AURÉA Perfumes - ${referencia}`,
      payment_method_id: "pix",
      external_reference: referencia,
      payer: { email, first_name: partes[0], last_name: partes.slice(1).join(" ") || "AUREA", identification: { type: "CPF", number: cpf } }
    };
    if (telefone.length >= 10) payload.payer.phone = { area_code: telefone.slice(0, 2), number: telefone.slice(2) };

    const mp = await fetch("https://api.mercadopago.com/v1/payments", {
      method: "POST",
      headers: { Authorization: `Bearer ${env.MERCADOPAGO_ACCESS_TOKEN}`, "Content-Type": "application/json", Accept: "application/json", "X-Idempotency-Key": referencia },
      body: JSON.stringify(payload)
    });
    const raw = await mp.text();
    let result; try { result = JSON.parse(raw); } catch { result = {}; }
    if (!mp.ok) {
      console.error("Mercado Pago:", JSON.stringify(result));
      return resposta({ ok: false, error: "O Mercado Pago não autorizou a criação do pagamento." }, mp.status >= 400 && mp.status < 600 ? mp.status : 502);
    }
    ...
    return resposta({ ok: false, error: "Erro interno ao criar o pagamento." }, 500);
  }
}
...
async function consultarPagamento(orderId, env) {
  ...
  if (!env.MERCADOPAGO_ACCESS_TOKEN) return resposta({ ok: false, error: "Pagamento temporariamente indisponível." }, 503);
  try {
    const mp = await fetch(`https://api.mercadopago.com/v1/payments/${encodeURIComponent(orderId)}`, { headers: { Authorization: `Bearer ${env.MERCADOPAGO_ACCESS_TOKEN}`, Accept: "application/json" } });
    const raw = await mp.text(); let result; try { result = JSON.parse(raw); } catch { result = {}; }
    if (!mp.ok) return resposta({ ok: false, error: "Não foi possível consultar o pagamento." }, mp.status >= 400 && mp.status < 600 ? mp.status : 502);
    return resposta({ ok: true, orderId: result.id ?? orderId, status: result.status ?? null, statusDetail: result.status_detail ?? null, paymentId: result.id ?? null });
  } catch (e) { return resposta({ ok: false, error: "Erro ao consultar pagamento." }, 500); }
}
...
function resposta(dados, status = 200) {
  return new Response(JSON.stringify(dados), { status, headers: jsonHeaders });
}
